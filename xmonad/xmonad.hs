...
myKeys conf@(XConfig {XMonad.modMask = modm}) = M.fromList $
    ...
    -- launch screenshot
    , ((controlMask .|. shiftMask, xK_4     ), spawn "flameshot gui -c -s")
    -- , ((controlMask .|. shiftMask, xK_4     ), spawn "gnome-screenshot -af /tmp/test.png && xclip /tmp/test.png -selection clipboard -target image/png; rm /tmp/test.png")

    -- launch dmenu
    , ((modm,               xK_p     ), spawn "rofi -display-drun '\xf002' -show drun ")
...
--
main = do
  xmproc <- spawnPipe ("xmobar -x 1 $HOME/.config/xmobar/xmobar.config")
  xmonad $ docks $ def {
      -- simple stuff
        terminal           = myTerminal,
...
--                             >> hPutStrLn xmproc x   -- xmobar on monitor 2
--               -- Current workspace
--             , ppCurrent = xmobarColor color06 "" . wrap
--                           ("<box type=Bottom width=2 mb=2 color=" ++ color06 ++ ">") "</box>"
--               -- Visible but not current workspace
