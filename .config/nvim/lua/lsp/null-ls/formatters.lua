...
local logger = require("log"):get_default()
...
function M.setup(filetype, options)
  if not O.lang[filetype] or (formatters_by_ft[filetype] and not options.force_reload) then
    return
  end

  formatters_by_ft[filetype] = M.list_configured(O.lang[filetype].formatters)
