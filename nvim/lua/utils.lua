...
function utils.reload_lv_config()
  vim.cmd "source ~/.local/share/lunarvim/O/lua/settings.lua"
  vim.cmd("source " .. USER_CONFIG_PATH)
  require("keymappings").setup() -- this should be done before loading the plugins
  vim.cmd "source ~/.local/share/lunarvim/O/lua/plugins.lua"
...
function utils.get_active_client_by_ft(filetype)
  local clients = vim.lsp.get_active_clients()
  for _, client in pairs(clients) do
    if client.name == O.lang[filetype].lsp.provider then
      return client
    end
  end
  return nil
